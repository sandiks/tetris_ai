  ...
  def self.anlz(map, piece_type)
    ...
    for i in 1..map.w
      ...
      ruls.each do |rl|
        ...
        found =case line
        when ['0', '0', '+']; [i, rr[i],    orient, 10] if rr[i] == rr[i+1] && rr[i+1] == rr[i+2]-hh[2]
        when ['+', '0'];      [i, rr[i+1],  orient, 9] if rr[i]-hh[0] == rr[i+1]
        when ['0', '0', '0']; [i, rr[i],    orient, 0] if rr[i] == rr[i+1] && rr[i+1] == rr[i+2]
